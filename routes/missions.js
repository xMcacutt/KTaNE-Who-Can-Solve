...
router.all("/", async (req, res) => {
  try {
    ...
    const { search, sort = "date_added", order = "descending", pack, author } = req.query;
    const orderQuery = order.toLowerCase() === "ascending" ? "ASC" : "DESC";
    const { team, discordId } = req.body;
    ...
    const missionsQuery = `
      ...
      ${whereClause};
    `;
    const result = await pool.query(missionsQuery, values);
    let missions = result.rows;

    if (sort === "known_modules") {
      if (!Array.isArray(team) || team.length === 0) {
        return res.status(400).json({ message: "Missing or invalid team data" });
      }
